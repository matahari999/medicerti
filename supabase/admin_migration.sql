...
-- 3. 기존 정책 삭제 후 재생성 (어드민 전체 접근)
DROP POLICY IF EXISTS "admin: 전체 병원 읽기" ON hospitals;
CREATE POLICY "admin: 전체 병원 읽기"
...
-- 확인
SELECT id, full_name, is_platform_admin FROM profiles WHERE is_platform_admin = true;
